...
interface DistrictRepository: JpaRepository<District, Int> {
    ...
    @Query(
            ...
            "FROM District dis LEFT JOIN Infected inf ON dis.city=inf.city AND true=ST_Contains(dis.geometry, ST_SRID(POINT(inf.lon, inf.lat), 4326)) " +
            "GROUP BY dis.id"
    )
    @Cacheable("district/analytics")
